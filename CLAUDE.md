...
Next (see TODO.md M22-M24): M22 query quality optimizations — code-syntax stopwords (+20-25% code_snippet est), operation-type inference (+20-30% request_payload est), endpoint search_text enrichment with param names, FTS/semantic query separation, score-aware linear fusion (+3-5% nDCG est). M23 structured endpoint extraction from crawled docs (reduce Postman/spec dependency). M24 content quality — Paradex URL drift, dYdX/Kraken thin pages, Bluefin login-gated, KuCoin nav pollution. Runtime repo synced. Pacifica re-evaluation when docs mature.
