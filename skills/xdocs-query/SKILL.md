...
- **Korean language support:** The classifier recognizes 18 Korean exchange names (바이낸스→binance, 바이빗→bybit, 오케이엑스→okx, etc.). Users can query in Korean natively. Korean exchange docs (Upbit, Bithumb, Coinone) are partially/fully in Korean; endpoint paths and parameter names are in English and searchable via FTS5. **Coinone is Korean-only** (no English docs exist). Cross-language search works: English queries find Korean pages via both FTS5 (endpoint paths/param names are English) and semantic search.
